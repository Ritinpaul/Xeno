        ...
        <div className="bloom-card p-6">
          ...
          {step === "confirm" && campaignResult && (
            <div className="space-y-5">
              ...
              {/* Message Preview - Channel Styled */}
              <div>
                <label className="block text-xs font-medium text-bloom-warm-gray mb-2">
                  AI-Generated Message Preview
                </label>
                <div className="rounded-xl border border-bloom-light-warm overflow-hidden">
                  {/* Channel header bar */}
                  <div className={`px-4 py-2.5 flex items-center gap-2 ${
                    selectedChannel === "whatsapp" ? "bg-[#075E54]" :
                    selectedChannel === "sms" ? "bg-[#3f51b5]" : "bg-bloom-brown"
                  }`}>
                    <ChannelIcon channel={selectedChannel} />
                    <span className="text-white text-xs font-semibold capitalize">{selectedChannel}</span>
                    <div className="ml-auto flex items-center gap-1.5">
                      <Sparkles className="w-3 h-3 text-white/70" />
                      <span className="text-white/70 text-[10px]">Variant {messageVariant + 1} · Personalised</span>
                    </div>
                  </div>
                  {/* Message bubble */}
                  <div className="p-4 bg-gray-50">
                    <div className="flex justify-end">
                      <div className={`message-bubble-${selectedChannel} max-w-[85%] animate-slide-up`}>
                        {campaignResult.messageVariants[messageVariant]}
                        <div className="text-right mt-1.5 text-[10px] opacity-50">
                  ...
                  {/* Variant switcher */}
                  <div className="px-4 py-2 border-t border-bloom-light-warm bg-white flex items-center gap-2">
                    <span className="text-[10px] text-bloom-warm-gray">Try another variant:</span>
                    {[0, 1, 2].map((v) => (
                      <button
                        key={v}
                        onClick={() => setMessageVariant(v)}
                        className={`w-6 h-6 rounded-full text-[10px] font-bold transition-all ${
                          messageVariant === v
                            ...
                            : "bg-bloom-cream text-bloom-warm-gray hover:bg-bloom-light-warm"
                        }`}
                      >
                        {v + 1}
                      </button>
